...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="linkedviewid"><xsl:value-of select="DisplayDevPacks:GetURIPatternPart($calcDocURI,'id')" /></xsl:variable>
	...
	<xsl:template match="root">
		<div id="divstepzero">
      <xsl:choose>
        <xsl:when test="(contains($docToCalcNodeName, 'budget')
              or contains($docToCalcNodeName, 'investment')
							or contains($docToCalcNodeName, 'devpack') 
							or contains($docToCalcNodeName, 'linkedview'))">
				</xsl:when>
				<xsl:otherwise>
					<h3>This budget and investment analyzer does not appear appropriate for the document being analyzed. Are you 
					sure this is the right analyzer?</h3>
				</xsl:otherwise>
			</xsl:choose>
      <h4 class="ui-bar-b"><strong>Budget and Investment M and E Analyzer Views</strong></h4>
      <h4 class="ui-bar-b">
					<strong>
	...
	<xsl:template match="linkedview">
        ...
        <div>
            ...
            <option>
						...
						<option>
							<xsl:attribute name="value">budget</xsl:attribute>
							<xsl:if test="(@FilesToAnalyzeExtensionType = 'budget')">
								<xsl:attribute name="selected" />
							</xsl:if>Operating Budgets
						</option>
						<option>
							<xsl:attribute name="value">investment</xsl:attribute>
							<xsl:if test="(@FilesToAnalyzeExtensionType = 'investment')">
								<xsl:attribute name="selected" />
							</xsl:if>Capital Budgets
						</option>
				  </select>
			  </div>
        <div>
     ...
     <div id="divsteptwo">
        ...
        <xsl:if test="($docToCalcNodeName = 'budgetgroup' or $docToCalcNodeName = 'investmentgroup'
							or $docToCalcNodeName = 'budget' or $docToCalcNodeName = 'investment'
							or contains($docToCalcNodeName, 'devpack'))">
