...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="linkedviewid"><xsl:value-of select="DisplayDevPacks:GetURIPatternPart($calcDocURI,'id')" /></xsl:variable>
	...
	<xsl:template match="root">
		<div id="divstepzero">
      <xsl:choose>
        <xsl:when test="(contains($docToCalcNodeName, 'output')
      ...
      <h4 class="ui-bar-b"><strong>Output M and E Analyzer Views</strong></h4>
      <h4 class="ui-bar-b">
					<strong>
	...
	<xsl:template match="linkedview">
     ...
     <div id="divsteptwo">
        ...
        <xsl:if test="($docToCalcNodeName = 'outputgroup' or $docToCalcNodeName = 'output'
							or contains($docToCalcNodeName, 'devpack'))">
